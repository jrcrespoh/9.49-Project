...
def get_sdr(model, loader, test_size, criterion, epoch, sdr_output_subset=None):
    """
    ...
    return all_sdrs, all_labels


class SequentialSubSampler(torch.utils.data.Sampler):
    """Custom sampler to take elements sequentially from a given list of indices.
...
def data_setup():
    ...
    if DATASET == "mnist":
        ...
        train_dataset = datasets.MNIST("data", train=True, download=True,
                                       transform=resize_and_normalize)
...
def build_mae_model(model, checkpoint_path):
    global_pool = True
    ...
    mae.interpolate_pos_embed(model, checkpoint_model)
    ...
    if global_pool:
        assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'fc_norm.weight', 'fc_norm.bias', 'k_winner.boost_strength', 'k_winner.duty_cycle'}
    else:
...
if __name__ == "__main__":
    ...
    results = get_sdr(model=sdr_mae, loader=train_loader, test_size=training_len,
    ...
    results = get_sdr(model=sdr_mae, loader=test_cnn_loader, test_size=testing_cnn_len,
    ...
    results = get_sdr(model=sdr_mae, loader=test_sdrc_loader,
